...
class TemplateController extends Controller
{
    ...
    {
        $site = Request()->get('_site');
        $template = Bookings::where('site', $site->id)->where('template', 1)->findOrFail($id);
    ...
    {
        $site = Request()->get('_site');
        $old = Bookings::where('site', $site->id)->where('template', 1)->findOrFail($id);
    ...
    {
        $site = Request()->get('_site');
        $booking = Bookings::where('site', $site->id)->where('template', 1)->findOrFail($id);
        $booking->name = $request->name;
        $booking->days = $request->days;
        $booking->site = $site->id;
